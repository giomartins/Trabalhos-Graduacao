...
# Funcoes da ELM com Poda e com H = tanh
trainELM_tanh <- function(X, Y, p, par, phi) {
  n <- dim(X)[2]
  if (par == 1){
  ...
  W <- matrix(0, nrow=dim(H)[2], ncol=max(Y))
  for (k in 1:max(Y)) {
    W[,k] <- pseudoinverse(H) %*% (Y == k)
  }
  retlist <- list(W, H, Z, linearize)
  return(retlist)
}

yELM_tanh <- function(X, Z, W, par, linearize){
  n <- dim(X)[2]
  if (par == 1){
    X <- cbind(1, X)
  }
...
# Funcoes da ELM com Poda e com H = sigm
trainELM_sigm <- function(X, Y, p, par, phi) {
  n <- dim(X)[2]
  if (par == 1){
    Z <- replicate(p, runif((n+1), -0.5, 0.5))
    X <- cbind(1, X)
...
yELM_sigm <- function(X, Z, W, par, linearize){
  n <- dim(X)[2]
  if (par == 1){
    X <- cbind(1, X)
  }
...
# Funcoes da ELM com Poda e com H = relu
trainELM_relu <- function(X, Y, p, par, phi) {
  n <- dim(X)[2]
  if (par == 1){
    Z <- replicate(p, runif((n+1), -0.5, 0.5))
    X <- cbind(1, X)
...
yELM_relu <- function(X, Z, W, par, linearize){
  n <- dim(X)[2]
  if (par == 1){
    X <- cbind(1, X)
  }
...
for (n in number_neurons){
  ...
  for (j in seq(1, 10)){
    ...
    result <- trainELM_tanh(X_training, Y_training_tanh, n, 1, phi)
    Z <- result[[3]]
    W <- result[[1]]
    linearize <- result[[4]]

    Yhat_train <- yELM_tanh(X_training, Z, W, 1, linearize)
    ...
    Yhat_test <- yELM_tanh(X_test, Z, W, 1, linearize)
  ...
  for (j in seq(1, 10)){
    ...
    result <- trainELM_sigm(X_training, Y_training_sigm, n, 1, phi)
    Z <- result[[3]]
    W <- result[[1]]
    linearize <- result[[4]]

    Yhat_train <- yELM_sigm(X_training, Z, W, 1, linearize)
    e_train <- sum((Y_training_sigm - Yhat_train)^2)/4
    e_train_percent <- e_train/length(Y_training_sigm)
    total_error_train[j] <- e_train_percent

    Yhat_test <- yELM_sigm(X_test, Z, W, 1, linearize)
  ...
  for (j in seq(1, 10)){
    ...
    result <- trainELM_relu(X_training, Y_training_relu, n, 1, phi)
    Z <- result[[3]]
    W <- result[[1]]
    linearize <- result[[4]]

    Yhat_train <- yELM_relu(X_training, Z, W, 1, linearize)
    e_train <- sum((Y_training_relu - Yhat_train)^2)/4
    e_train_percent <- e_train/length(Y_training_relu)
    total_error_train[j] <- e_train_percent

    Yhat_test <- yELM_relu(X_test, Z, W, 1, linearize)
...
plot(number_neurons, M[,2], type='l', lwd= 3, col='red', ylim = c(0, max(M)), ylab = 'M (Erro - ELM Podada)', xlab = 'Numero de Neuronios')
par(new = T)
plot(number_neurons, M[,4], type='l', lwd= 3, col='blue', ylim = c(0, max(M)), ylab = 'M (Erro - ELM Podada)', xlab = 'Numero de Neuronios')
par(new = T)
plot(number_neurons, M[,6], type='l', lwd= 3, col='green', ylim = c(0, max(M)), ylab = 'M (Erro - ELM Podada)', xlab = 'Numero de Neuronios')
par(new = T)
...
plot(number_neurons, M[,1], type='l', lwd= 3, col='red',, ylim = c(0, 2), ylab = 'M (Erro)', xlab = 'Numero de Neuronios' )
par(new=T)
plot(number_neurons, M[,2], type='l', lwd= 3, col='pink', ylim = c(0, 2), ylab = 'M (Erro)', xlab = 'Numero de Neuronios')
par(new = T)
plot(number_neurons, M[,3], type='l', lwd= 3, col='black', ylim = c(0, 2), ylab = 'M (Erro)', xlab = 'Numero de Neuronios' )
par(new=T)
plot(number_neurons, M[,4], type='l', lwd= 3, col='gray', ylim = c(0, 2), ylab = 'M (Erro)', xlab = 'Numero de Neuronios')
par(new = T)
plot(number_neurons, M[,5], type='l', lwd= 3, col='blue',, ylim = c(0, 2), ylab = 'M (Erro)', xlab = 'Numero de Neuronios' )
par(new=T)
plot(number_neurons, M[,6], type='l', lwd= 3, col='lightblue', ylim = c(0, 2), ylab = 'M (Erro)', xlab = 'Numero de Neuronios')
par(new = T)
