...
# Epsilon movido para o final da linha de argumentos, ele deve ser um parâmetro separado do gap
def algpint_01(A, B, C, REL, X0, XMIN, XMAX, gap, KALPHA=None, MAXITER=None, epslon=None):
    ...
    # Specified value considered zerona
    valzero = epslon ** 3
    ...
    while ITER <= MAXITER and FLAG1 == 0:
        ...
        if ITER != 1:
            DELTAX = X - XOLD
            #print("deltax ", DELTAX)
            if np.max(np.abs(DELTAX)) < epslon:     # Epsilon usado somente aqui
                XOLD = X
                FLAG1 = 1
            ...
            if gap_final < gap:    # O gap deve ser comparado com o gap especificado, mas não o DELTAX
                FLAG1 = 1
