...
% Algoritmos Quase Netwton - BFGS
  function [XK,Hfobj,k,kgold,icfunc]= otqnmat_a77(funcao,isa_FV,Imetqn,MAXITER,x0,Xmin,Xmax,epslon)
% ------------------------------------------------------------------
  ...
  while (~fim)
      ...
      % Norma vetor gradiente para função objetivo é nula.
        if norm(dfobjnew) <= keps*epslon
           disp('    Norma do gradiente função objetivo é nula.  '),
           return
        end
       
      % -------------------------------------------------
      % Gera a matriz aproximação para a inversa da hessiana (BFGS).
      ...
      % Define direção de busca com vetor unitário
        mdxk = max(abs(dxk));
        if mdxk <= keps*epslon
           disp('    O módulo do vetor direção de busca é nulo. '),
           return
        else
      ...
      % Verificar parada pelo número máximo de iterações.
        if k >= MAXITER
           fim = 1;
           disp('   Parada pelo número máximo de iterações.'),
        end
