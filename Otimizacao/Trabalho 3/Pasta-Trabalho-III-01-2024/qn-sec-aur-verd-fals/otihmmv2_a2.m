...
% Calcula uma aproximação para a hessiana ou a inversa da hessiana (MMV).
  function [IHk,IER]=otihmmv2_a2(tetha,pho,fold,fnew,gkold,gknew,xkold,...
                                 xknew,IHk,IIHES,Icorr)
% ----------------------------------
...
%
% Icorr : Método Quasi-Newton a ser usado e correçao para funcoes nao quadraticas.
%      0 : DFP
%      1 : BFGS
...
% Verifica se matriz é positivo definida ou não (Decomp.CHOLESKY).
  ...
  if IER ~= 0   
     if Icorr == 1
        IHk = IHkback;
        vety = gknew - gkold;%volta ao BFGS
        % Calcula uma aproximação para a matriz inversa da Hessiana.
        if IIHES == -1
           sig = (vetp')*vety;
           tau = (vety')*IHk*vety;
           ...
           IHk = (IHk - IHk*vety*(vety')*IHk/tau + tetha*(tau*vetp*(vetp')/sig^2 - ...
                  IHk*vety*(vetp')/sig - vetp*(IHk*vety)'/sig + ...
                  IHk*vety*(vety')*IHk/tau))*pho + vetp*(vetp')/sig;         
        end      
        ...
        if IIHES == 1
            ...
            IHk = (IHk - IHk*vetp*(vetp')*IHk/tau + tetha*(tau*vety*(vety')/sig^2 - ...
                   IHk*vetp*(vety')/sig - vety*(IHk*vetp)'/sig + ...
                   IHk*vetp*(vetp')*IHk/tau))*pho + vety*(vety')/sig;
        end      
     end
  end
% ----------------------------------------------------------------
% Verifica se matriz é positivo definida ou não (Decomp.CHOLESKY).
  [IER]= otdchol(IHk,epslon);
  if IER ~= 0
     fprintf(' Problema   Decomp. Cholesky  '),
